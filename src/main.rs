//! Projet de génération procédurale type Minecraft avec Bevy Engine
//!
//! # Fonctionnalités
//! - Génération de terrain avec seed
//! - Caméra FPS
//! - UI menu avec gestion des états
//! - Simulation physique avec timestep fixe
//!
//! # Architecture
//! - "WorldMap" : stockage des blocs
//! - "generate_blocks" : génération procédurale
//! - "display_blocks" : rendu
//! - "GameState" : gestion Menu / InGame
...
/// Flag indiquant si le timestep fixe a tourné ce frame (Code Bevy)
...
impl BlockType {
    /// Retourne la couleur associée au type de bloc.
    ///
    /// # Détails
    /// Permet de définir un matériau simple sans texture.
    ///
    /// # Mapping
    /// - Dirt  → marron
    /// - Wood  → marron clair
...
/// Tag pour identifier le sol de la scène.
#[derive(Component)] struct Ground;
/// Tag pour identifier les blocs affichés dans le monde.
#[derive(Component)] struct WorldBlock;
/// Tag pour tous les éléments UI du menu principal.
#[derive(Component)] struct MenuUI;
/// Tag pour tous les éléments UI affichés en jeu.
#[derive(Component)] struct InGameUI;
/// Tag pour le texte affichant la seed dans le menu.
#[derive(Component)] struct SeedText;

/// Actions possibles associées aux boutons du menu.
#[derive(Component)]
enum MenuButton { Play, NewSeed, Quit }

/// Input de déplacement accumulé entre deux timesteps (Code Bevy)
#[derive(Debug, Component, Clone, Copy, PartialEq, Default, Deref, DerefMut)]
struct AccumulatedInput { movement: Vec2 }

/// Vélocité du joueur dans la simulation physique (Code Bevy)
#[derive(Debug, Component, Clone, Copy, PartialEq, Default, Deref, DerefMut)]
struct Velocity(Vec3);

/// Position physique réelle du joueur (séparée du Transform visuel) (Code Bevy)
#[derive(Debug, Component, Clone, Copy, PartialEq, Default, Deref, DerefMut)]
struct PhysicalTranslation(Vec3);

/// Position physique du frame précédent (pour l'interpolation) (Code Bevy)
#[derive(Debug, Component, Clone, Copy, PartialEq, Default, Deref, DerefMut)]
struct PreviousPhysicalTranslation(Vec3);

/// Sensibilité de la caméra (x = horizontal, y = vertical) (Code Bevy)
...
/// ///
/// # Variants
/// - "Menu" : affiche le menu principal avec l'UI
/// - "InGame" : lance la simulation et affiche le monde 3D
///
/// # Utilisation
/// Cet état est utilisé avec le système de states de Bevy pour :
/// - activer/désactiver des systèmes
/// - gérer le cycle de vie (OnEnter / OnExit)
...
/// Génère un terrain procédural à partir d'une graine.
///
/// # Description
/// Cette fonction remplit la "WorldMap" avec des blocs en utilisant :
/// - un bruit sinusoïdal (relief lisse)
/// - une composante aléatoire (variation locale)
///
/// # Paramètres
/// - "world" : ressource contenant tous les blocs
/// - "seed" : graine déterministe pour la génération (identifiant du monde)
///
/// # Détails
/// - Deux fonctions trigonométriques ("noise1", "noise2")
/// - Un offset aléatoire casse la régularité
/// - La hauteur est bornée à 0 minimum
///
/// # Structure du terrain
/// - Surface : bois ou pierre
/// - Sous-sol : terre majoritaire
/// - Profondeur : pierre dominante
...
/// Affiche tous les blocs du monde.
///
/// # Optimisation
/// Un seul mesh de cube est créé puis cloné pour tous les blocs.
///
/// # Paramètres
/// - "commands" : permet de spawn les entités
/// - "world" : contient les blocs à afficher
/// - "meshes" : ressource des meshes
/// - "materials" : ressource des matériaux
fn display_blocks(
...
/// Initialise la scène 3D de base.
///
/// # Contenu
/// - Un sol (plan) servant de référence visuelle
/// - Une lumière directionnelle pour éclairer la scène
///
/// # Détails
/// Cette fonction est appelée lors de l'entrée dans l'état "InGame".
///
/// # Remarque
/// Les blocs du monde sont générés séparément via "generate_blocks"
/// puis affichés avec "display_blocks".
fn setup(
    mut commands: Commands,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
...
// ============================================================
// JOUEUR ET CAMÉRA (Code Bevy)
// ============================================================

/// Initialise le joueur et la caméra.
///
/// # Détails
/// - Vue FPS
/// - Le joueur est une entité physique invisible
///
/// # Composants ajoutés
/// - `AccumulatedInput` : input utilisateur
/// - `Velocity` : vitesse actuelle
/// - `PhysicalTranslation` : position physique
/// - `PreviousPhysicalTranslation` : pour interpolation
...
/// # Détails
/// - Axe horizontal : yaw
/// - Axe vertical : pitch (limité)
///
/// # Sécurité
/// Le pitch est clampé pour éviter :
/// - retournement de caméra
/// - comportements incohérents
///
/// # Paramètres
/// - "AccumulatedMouseMotion" : mouvement souris accumulé
fn rotate_camera(
...
///
/// # Détails
/// - Le mouvement est relatif à la caméra
/// - La vitesse est normalisée pour éviter les diagonales plus rapides
///
/// # Paramètres
/// - "keyboard_input" : état des touches
/// - "player" : composants input + vitesse
/// - "camera" : orientation actuelle
fn accumulate_input(
...
/// - Sauvegarde la position précédente (interpolation)
///
/// # Important
/// Fonction exécutée dans "FixedUpdate"
fn advance_physics(
...
///
/// # Détails
/// Permet d'éviter les saccades dues au fixed timestep.
///
/// # Paramètres
/// - "alpha" : fraction d'avancement entre deux steps
...
/// Crée un bouton UI avec un label et une action.
///
/// # Paramètres
/// - "parent" : conteneur UI parent
/// - "label" : texte affiché
/// - "action" : action associée au bouton
///
/// # Détails
/// - Ajoute automatiquement un enfant "Text"
/// - Associe un composant "MenuButton"
...
/// # Actions
/// - Play → passe en jeu
/// - NewSeed → génère une nouvelle graine
/// - Quit → retourne au menu
...
/// Initialise l'interface utilisateur en jeu.
///
/// # Contenu
/// - Bouton "Quit" permettant de revenir au menu
///
/// # Position
...
/// Gère les interactions des boutons en jeu.
///
/// # Actions
/// - Quit → retour au menu principal
///
/// # Détails
/// Change également la couleur du bouton selon l'état :
/// - Hovered
/// - Pressed
/// - None
